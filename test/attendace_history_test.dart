  ...
  group("Attendance Histort testing", () {
    ...
    test(
      ...
      () async {
        ...
        var response = await attendanceHistoryService.getAllHistory(
            month: "10", year: "2024", token: mockToken);
        expect(response, isA<List<AttendanceHistoryModel>>());
      },
    );

    test(
      "get all months end point testing",
      () async {
        final client = MockClient();
        when(client.get(
                Uri.parse(APIConstants.BASE_URL + APIConstants.MONTH_LIST)))
            .thenAnswer((_) async => http.Response(
                ' {"results":[{"month":"11","year":"2024","attendance_count":"2"},{"month":"12","year":"2024","attendance_count":"8"},{"month":"1","year":"2025","attendance_count":"7"}],"success":true}',
                200));

        var response =
            await attendanceHistoryService.getAllMonths(token: mockToken);
        expect(response, isA<List<MonthListModel>>());
      },
    );

    test(
      "get all months end point exception testing",
      () async {
        final client = MockClient();
        when(client.get(
                Uri.parse(APIConstants.BASE_URL + APIConstants.MONTH_LIST)))
    ...
    test(
      ...
      () async {
        ...
        try {
          var response = await attendanceHistoryService.getAllHistory(
              month: "", year: "2024", token: mockToken);
          expect(response, throwsException);
        } catch (e) {
          expect(e.toString(), 'Exception: Failed to Mark attendace');
        }
