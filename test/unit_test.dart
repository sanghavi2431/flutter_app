  ...
  group("Test Login Module Endpoint API calls", () {
    ...
    test('Test send otp endpoints ', () async {
      ...
      when(client.post(body: {
        "mobileNumber": "8097267015",
      }, Uri.parse(APIConstants.BASE_URL + APIConstants.SEND_OTP)))
          .thenAnswer((_) async => http.Response(
              '   "results": {  "request_id": "8bac61ad-2926-4821-8a77-2113f49b4491"  },"success": true',
              200));
      ...
      when(client.post(body: {
        "mobileNumber": "",
          ...
          .thenAnswer((_) async =>
              ...
              http.Response(
                  '{"message": "mobileNumber" must be a number, "success": false, "results": []}',
                  400));
    ...
    test('testing otp  api endpoints', () async {
      final client = MockClient();
      when(client.post(body: {
        "request_id": "6de27dd6-e3f9-4b7b-8e14-3a5aeda1db3f",
        "otp": "1234"
      }, Uri.parse(APIConstants.BASE_URL + APIConstants.VERIFY_OTP)))
          .thenAnswer((_) async => http.Response(
              '  "results": { "name": "shrirang  jangam",  "mobile": "8097267015", "id": 419, "role_id": 2,  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6NDE5LCJyb2xlX2lkIjoyLCJpYXQiOjE3MzA3NzQ1OTUsImV4cCI6MTczMTM3OTM5NX0.cHWD0idDyb4NNRaFQHl05caNYz-9CzyWp6gW3_XKxWs"  }, "success": true',
              200));

      expect(await loginService.verifyOTP(otp: "1234", requestId: reqId),
          isA<VerifyOtpModel>());
    });
      ...
      when(client.post(body: {
        ...
        "otp": "1234"
          ...
          .thenAnswer((_) async =>
              // http.Response('Not Found', 400));
              http.Response(
                  '{message: "otp" is not allowed to be empty, success: false, results: []}',
                  200));
