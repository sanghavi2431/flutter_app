...
void main() {
  ...
  group("task list  testing", () {
    ...
    test(
      " get all task  end point testing",
      () async {
        final client = MockClient();
        when(client.post(body: {
          "id": taskId,
        }, Uri.parse(APIConstants.BASE_URL + APIConstants.GET_ALL_TASKS)))
            .thenAnswer((_) async => http.Response(
                '{"results": { "template_id": "267", "tasks": [{  "task_id": "97", "task_name": "Morning Cleaning"  }  ] }, "success": true}',
                200));

        var response =
            await taskListService.getAllTasks(id: taskId!, token: mockToken);
    ...
    test(
      " get all task  end point testing fail",
      () async {
        final client = MockClient();
        when(client.post(body: {
          "id": taskId,
        ...
        try {
          var response =
              await taskListService.getAllTasks(id: taskId!, token: "");
      ...
      () async {
        ...
        when(client.post(body: {
          "data": "fake data",
            ...
            .thenAnswer((_) async => http.Response(
                ' { "results": "Task submitted!","success": true}', 200));

        var response = await taskListService.submitTask(
            token: mockToken,
            createTaskModel: CreateTaskModel(allocationId: allocationId, data: [
              InternalData(
                status: 1,
                taskId: "97",
      ...
      () async {
        ...
        when(client.post(body: {
          "data": "",
            ...
            .thenAnswer((_) async => http.Response(
                '{message: "allocation_id" must be a number, success: false, results: []}',
                400));
        ...
        try {
          await taskListService.submitTask(
              token: mockToken,
              createTaskModel: CreateTaskModel(allocationId: '', data: [
                InternalData(
                  status: 1,
                  taskId: "97",
                  taskName: "Morning Cleaning",
    ...
    test(
      "  update all task  end point testing",
      () async {
        final client = MockClient();
        when(client.post(body: {
          "id": taskId,
        ...
        var response = await taskListService.updateStatus(
            id: taskId.toString(), status: '3', token: mockToken);
