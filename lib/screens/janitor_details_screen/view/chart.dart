...
class _ChartState extends State<Chart> {
  ...
  void initState() {
    ...
    var temp = double.parse(widget.complatedTask!) /
        double.parse(widget.totalTask!) *
        100;

    compaltedPer = double.parse(temp.toStringAsFixed(2));

    var other = double.parse(widget.pendingTask!) /
        double.parse(widget.totalTask!) *
        100;

    pendingPer = double.parse(other.toStringAsFixed(2));

    double accpet = double.parse(widget.accetedTask!) /
        double.parse(widget.totalTask!) *
        100;

    acceptedPer = double.parse(accpet.toStringAsFixed(2));

    double reject = double.parse(widget.rejectedTask!) /
        double.parse(widget.totalTask!) *
        100;
    ...
    double rfc =
        double.parse(widget.rfcTask!) / double.parse(widget.totalTask!) * 100;

    rfcPer = double.parse(rfc.toStringAsFixed(2));

    double ongoing = double.parse(widget.ongoingTask!) /
        double.parse(widget.totalTask!) *
        100;
    ...
    pies = [
      PieData(
        value: compaltedPer == 0 ? 0.01 : compaltedPer!,
        color: const Color(0xff006C7B),
      ),
      PieData(value: pendingPer == 0 ? 0.01 : pendingPer!, color: Colors.cyan),
      PieData(
          value: acceptedPer == 0 ? 0.01 : acceptedPer!,
          color: AppColors.acceptedBgColor),
      PieData(
          value: rejectedPer == 0 ? 0.01 : rejectedPer!,
          color: AppColors.rejectButtonColor),
      PieData(
          value: rfcPer == 0 ? 0.01 : rfcPer!, color: AppColors.rfcCardBgColor),
      PieData(
          value: onGoingPer == 0 ? 0.01 : onGoingPer!,
