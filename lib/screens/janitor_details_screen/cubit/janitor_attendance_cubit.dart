...
class JanitorAttendanceCubit extends Cubit<JanitorAttendanceState> {
  final JanitorAttendanceService _service =
      JanitorAttendanceService(dio: GetIt.instance());
  final int _janiId;
  JanitorAttendanceCubit(this._janiId)
      : super(const JanitorAttendanceInitial([], [], null, null, ''));
  ...
  void getMonth(MonthListModel month) async {
    try {
      emit(JanitorAttendanceLoading(
          state.months, state.attendance, month, null, ''));
      final attendance = await _service.getAllHistory(
          month: month.month ?? "", year: month.year ?? "", janiId: _janiId);
  ...
  void sort(String? sortBy) async {
    try {
      emit(JanitorAttendanceLoading(
          state.months, state.attendance, state.selected, sortBy, ''));
      if (sortBy == null) {
      ...
      state.attendance.sort((a, b) {
        ...
        } else if (a.attendance?.toLowerCase() != sortBy &&
            b.attendance?.toLowerCase() == sortBy) {
          return 1; // b comes first
        } else {
          return 0; // maintain original order
        }
      });
      emit(JanitorAttendanceSuccess(
          state.months, state.attendance, state.selected, sortBy, ''));
    } catch (e) {
      emit(JanitorAttendanceError([], [], null, null, e.toString()));
      Logger().e(e);
