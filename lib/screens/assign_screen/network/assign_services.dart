...
class AssignService {
  ...
  Future<List<DashboardModelClass>> getTasksByJanitorId(int id,
      {String? token}) async {
    try {
  ...
  Future<List<JanitorListModel>> getanitorListByFacilityId(int facilityId,
      {String? token}) async {
    try {
      List<JanitorListModel> output = [];
      var response = await dio.post(
        APIConstants.JANITOR_LIST_FACILITY,
        data: {"facility_id": facilityId, "role_id": 1},
        options: token != null
            ? Options(
