    ...
    return BlocConsumer(
      ...
      builder: (context, state) {
        ...
        if (state is SupervisorDashboardLoading) {
          ...
          EasyLoading.dismiss();
          _data = state.data.where((e) {
            return e.status == widget.status && e.requestType == widget.reqType;
          }).toList();
                    ...
                    children: [
                      SizedBox(
                        ...
                        child: CardSwiper(
                          ...
                          numberOfCardsDisplayed: _data.length == 1 ? 1 : 2,
...
// backgroundCardCount: _data.length,
                                                                      ...
                                                                      (context) =>
                                                                          TaskDetailsScreen(
                                                                            ...
                                                                            isFromFacility:
                                                                                false,
                                                                            allocationId:
                                                                                _data[index].taskAllocationId.toString(),
                                                                            isApproved:
                                                                                isApproved,
                                                                            tabController:
                                                                                widget.tabController,
                                                                            isTaskImageRequired:
                                                                                _data[index].requiredTaskImage,
                                                                          )),
                                                            );
        ...
        } else if (state is SupervisorUpdateStatusLoading) {
          EasyLoading.show(
              status: MydashboardScreenConstants.LOADING_TOAST.tr());
        } else if (state is SupervisorUpdateStatusSuccessful) {
          EasyLoading.dismiss();

          isApproved = true;

          debugPrint("SupervisorUpdateStatusSuccessful $isApproved");
        } else if (state is SupervisorUpdateStatusError) {
          EasyLoading.dismiss();
