...
class ReportIssueService {
  ...
  Future<List<TaskNamesModels>> getTasksDropdownData(int clusterId) async {
    try {
  ...
  Future<TaskListModel> getAllTasksList({required String id}) async {
    try {
      // accept-language will be added by AuthInterceptor for auth requests
      var response = await dio.get(
        ...
        MapEntry(
          ...
          await MultipartFile.fromFile(
            ...
            contentType: MediaType(
                getType(taskImages.path), getFileExtension(taskImages.path)),
          ),
        ),
      ]);
