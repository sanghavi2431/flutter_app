  ...
  Widget build(BuildContext context) {
    return BlocConsumer(
        ...
        builder: (context, state) {
                        ...
                        child: RefreshIndicator(
                          onRefresh: () {
                            return Future.delayed(
                              ...
                              () {
                                taskListBloc.add(
                                    GetAllTask(id: widget.templateId ?? 0));
                              },
                            );
                          },
                            ...
                            itemBuilder: (
                              ...
                              return Padding(
                                ...
                                child: CheckboxListWidget(
                                  name: data.tasks?[index].translatedTaskName ??
                                      data.tasks?[index].taskName,
                                  isChecked: createTaskModel.data!
          ...
          } else if (state is SubmitTasksLoading) {
            EasyLoading.show(
                status: MydashboardScreenConstants.LOADING_TOAST.tr());
          } else if (state is SubmitTasksError) {
            if (kDebugMode) {
              print(state);
            }

            return CustomErrorWidget(error: state.error.message);
          } else if (state is SubmitTasksSuccess && (state.data.isEmpty)) {
