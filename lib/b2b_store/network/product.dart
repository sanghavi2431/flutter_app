...
class ProductService {
  ...
  Future<ProductCollections> getProductCollectionsById({
    ...
    try {
      ...
      var response = await dio.get(
        "https://staging-store.woloo.in/store/products?fields=*variants.calculated_price,+variants.inventory_quantity,*categories&$slug=$id",
        options: Options(
          headers: {
            'Content-Type': 'application/json',
            'x-publishable-api-key':
  ...
  Future<ProductCollections> searchProducts(
    ...
    try {
      var response = await dio.get(
        "https://staging-store.woloo.in/store/products?fields=*variants.calculated_price%2C+variants.inventory_quantity,*categories&region_id=$regionId&q=$query",
        options: Options(
          headers: {
            'Content-Type': 'application/json',
            'x-publishable-api-key':
  ...
  Future<ProductCollections> getProductsByCategoryId({
    ...
    try {
      var url = sized == null
          ? "https://staging-store.woloo.in/store/products?fields=*variants.calculated_price,*categories%2C+variants.inventory_quantity&category_id=$categoryId"
          : "https://staging-store.woloo.in/store/products?fields=*variants.calculated_price,*categories%2C+variants.inventory_quantity&category_id=$categoryId&variants[options][value]=$sized";
      // https://staging-store.woloo.in/store/products?fields=*variants.calculated_price, variants.inventory_quantity&category_id=pcat_01JPH852CKBG58XW4F482EZAS0&variants[options][value]=Large
      log("getProductsByCategoryId URL: $url");

      var response = await dio.get(
        url,
        // "https://staging-store.woloo.in/store/products?fields=*variants.calculated_price%2C+variants.inventory_quantity&category_id=$categoryId&variants[options][value]=$sized",
        options: Options(
          headers: {
            'Content-Type': 'application/json',
            'x-publishable-api-key':
  ...
  Future<ProductCollections> getAllProducts({
    required token,
    required String regionId,
  }) async {
    // query = Uri.encodeComponent(query);
    try {
      var response = await dio.get(
        "https://staging-store.woloo.in/store/products?fields=*variants.calculated_price,*categories variants.inventory_quantity,*categories&region_id=$regionId",
        options: Options(
          headers: {
            'Content-Type': 'application/json',
            'x-publishable-api-key':
...
// https://staging-store.woloo.in/store/products?fields=*variants.calculated_price, variants.inventory_quantity,*categories&region_id=reg_01JPH693TAM20TXZEJNBJ5QBV4
