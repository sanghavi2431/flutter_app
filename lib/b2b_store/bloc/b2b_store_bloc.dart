...
class B2bStoreBloc extends Bloc<B2BStoreEvent, B2BStoreState> {
  ...
  B2bStoreBloc() : super(B2BStoreInitial()) {
    ...
    on<CheckInventory>(checkInventory);
    on<GetDeliveryPartners>(getDeliveryPartners);
      ...
      await loginFlowService
        ...
        response =
            await loginFlowService.createCustomer(email: event.email, token: v);
                  await  loginFlowService.updateCustomer(token: v);
            // .then( (r) async{
  ...
  FutureOr<void> _refresh(
    ...
    try {
      ...
      CartModel cartModel = await _cartService.getAllCartData(
       ...
       await  loginFlowService.getProfile(
          //  clientUserId: decodedToken["id"],
  ...
  FutureOr<void> _placeOrder(
    ...
    try {
      ...
      await _productService
          ...
          .then((cartData) {
        print("card fate ${cartData.cart.id}");
        box.write('cart_id', cartData.cart.id);
      });

      print("compalet venrew $completeVendor");
...
//TODO:implement{curl -X POST -H "user-agent: Android/22110/10" -H "x-woloo-token: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6NjMxNjAsImlhdCI6MTc1MDk0MzI2NywibmJmIjoxNzUwOTQzMjY3LCJleHAiOjE3NTg3MTkyNjcsImlzcyI6Imh0dHBzOi8vd29sb28udmVyaWZpbm93LmNvbS9hcGkvdjEvbG9naW4iLCJzdWIiOiI2MzE2MCIsImp0aSI6IkFCQ0RFRkdISUpLIn0.GMP72gC_qLaBc39i-Nelv8bI3-iLKfE7FlrTu2RULTw" --data $'{"coins":10,"orderid":"ordset_01JYP5PDX9FZK1AT8R88WHJ9VD","type":"points"}' https://staging-api.woloo.in/api/blog/ecomCoinUpdate}
      ...
      print("is woloo pints $isWolooPointsApplied");
  ...
  FutureOr<void> _getAllProducts(
    ...
    try {
      ...
      final response = await _productService.getAllProducts(
        ...
        // query: event.categoryId,
      );
      print("All Products Response: $response");
      emit(AllProductsSuccess(productCollections: response));
    } catch (e) {
      emit(ProductsByCategoryError(error: e.toString()));
      debugPrint("Error in get products by category service: $e");
    }
  ...
  FutureOr<void> updateaddress(
    ...
    try {
      ...
      //   categoryId: event.categoryId,
      // );
      emit(UpdateAddressSuccess(addressesData: event.addressesData));
    } catch (e) {
      emit(ProductsByCategoryError(error: e.toString()));
      debugPrint("Error in get products by category service: $e");
    }
  ...
  FutureOr<void> checkInventory(
    ...
    try {
      emit(const ProductsByCategoryLoading(
          message: "Loading products by category..."));
      final response = await _cartService.checkInventory(
        cartId: box.read('cart_id'),
        token: box.read('login_jwt'),
      );
      logger.w("Check Inventory  Response in bloc : $response");
      emit(CheckInventorySuccess(inventoryModel: response));
    } on DioException catch (e) {
      emit(CheckInventoryError(error: e.toString()));
      debugPrint("Error in get products by category service: $e");
    }
  }

  FutureOr<void> getDeliveryPartners(
    GetDeliveryPartners event,
    Emitter<B2BStoreState> emit,
  ) async {
    try {
      emit(const DeliveryPartnersLoading());

      final response = await _cartService.getDeliveryPartnersListFromPincode(
        fields: event.pincode,
      );

      print("res in pinocde $response ");
      emit(DeliveryPartnersSuccess(response: response));
    } on DioException catch (e) {
      emit(DeliveryPartnersError(error: e.toString()));
      debugPrint("Error in getDeliveryPartners service: $e");
    }
  }
  ...
  FutureOr<void> getCartData(
    ...
    emit(const CartLoading(message: "Loading..."));
    try {
      // //logger.w(response);
      CartModel response = await _cartService.getAllCartData(
          token: box.read('login_jwt'), cartId: box.read('cart_id'));

      emit(CartSuccess(cartData: response, wolooPoints: 0));

      // print("res in pinocde $response ");
      // emit(DeliveryPartnersSuccess(response: response));
    } on DioException catch (e) {
      emit(CartError(error: e.toString()));
      debugPrint("Error in getCartData service: $e");
    }
  }
}
