...
import 'package:woloo_smart_hygiene/hygine_services/model/hygiene_categories.dart';
...
class HygieneServiceBloc
  ...
  HygieneServiceBloc() : super(HygieneServiceInitial()) {
    ...
    on<GetCategory>(_getHygieneCategories);
  }
  ...
  FutureOr<void> _getHygieneCategories(
    GetCategory event,
    Emitter<HygieneServiceState> emit,
  ) async {
    try {
      emit(const HygieneCartLoading(message: "Loading data..."));
      HygieneCategories response =
          await hygieneServiceApi.getHygieneCategories();

      debugPrint("requestId $response");
      print(response);

      emit(HygieneCategorySuccess(serviceCategories: response));
    } catch (e) {
      emit(HygieneCartError(error: e.toString()));
      debugPrint("Error in hygiene category service: $e");
    }
  }
}
