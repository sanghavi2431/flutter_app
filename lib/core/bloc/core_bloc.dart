...
class CoreBloc extends Bloc<CoreEvent, CoreState> {
  ...
  CoreBloc() : super(CoreInitial()) {
    ...
    on<CheckForceUpdateEvent>(_mapForceUpdateToState);
  }
  ...
  FutureOr<void> _mapForceUpdateToState(
      CheckForceUpdateEvent event, Emitter<CoreState> emit) async {
    try {
      emit(ForceUpdateLoading());
      //    var token = globalStorage.getToken();
      //  print( " from strage $token");
      //   if (token.isNotEmpty) {
      var response = await coreService.checkForceUpdate();
      //   }
      //

      print("response: $response");

      emit(ForceUpdateSuccess(forceUpdate: response
  ...
  FutureOr<void> _mapGetClientState(
    ...
    try {
      ...
      var response = await coreService.getClient(id: event.id);

      GlobalStorage globalStorage = GetIt.instance();
      globalStorage.saveClientId(accessClientId: response.results!.client!.value.toString());

      debugPrint("requestId from coreBloc $response");
      //  = response;

      emit(ClientSuccess(model: response
          // subscriptionModel:  response,
          ));
    } catch (e) {
      emit(UpdateTokenError(error: ErrorHandler.handle(e).failure));
