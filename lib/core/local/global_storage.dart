...
// import '../../b2b_store/models/order_details.dart';
...
class GlobalStorage {
  ...
  final String _recentSearch = 'recentSearch';
  final String _address = 'defaultAddress';
  final String _wolooId = 'wolooId';
  final String _languageCodes = 'languageCodes';
  ...
  void removeSupervisorName() {
   ...
   saveMobileNumber({required String accessMobileNumber}) {
  ...
  void saveAddress({required Addresses accessAddress}) {
    if (accessAddress.toString().isEmpty) {
  ...
  Addresses getAddress() {
    Addresses? address = _box.read(_addressKey);
  ...
  void saveRecentSearch({required List<dynamic> recentSearch}) {
    if (recentSearch.isEmpty) {
      throw 'Recent Search is empty';
    }
    _box.write(_recentSearch, recentSearch);
  }

  List<dynamic> getRecentSearch() {
    List<dynamic> recentSearch = _box.read(_recentSearch) ?? [];
    // print("resnd $recentSearches");
    return recentSearch ?? [];
  }

  void removeRecentSearch() {
    _box.remove(_recentSearch);
  }

  void saveWolooId({required String wolooId}) {
    if (wolooId.isEmpty) {
      throw 'Woloo Id is empty';
    }
    _box.write(_wolooId, wolooId);
  }

  String getWolooId() {
    return _box.read(_wolooId) ?? '';
  }

  void removeWolooId() {
    _box.remove(_wolooId);
  }

  void saveLanguageCodes({required List<String> languageCodes}) {
    _box.write(_languageCodes, languageCodes);
  }

  List<String> getLanguageCodes() {
    List<dynamic>? codes = _box.read(_languageCodes);
    if (codes == null) return [];
    return codes.cast<String>();
  }

  void removeLanguageCodes() {
    _box.remove(_languageCodes);
  }

  // void saveAddress({required List<dynamic> recentSearch}) {
  //   if (recentSearch.isEmpty) {
  //     throw 'Recent Search is empty';
  //   }
  //   _box.write(_recentSearch, recentSearch);
  ...
  // List<dynamic> getAddress() {
  //   List<dynamic> recentSearch =

  //   _box.read(_recentSearch)  ?? [];
  //       // print("resnd $recentSearches");
  //   return recentSearch ?? [];
  // }
