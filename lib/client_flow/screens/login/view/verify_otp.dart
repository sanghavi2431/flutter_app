...
class _VerifyOtpState extends State<VerifyOtp> {
  ...
  bool _isOtpVerified = false; // Flag to track if OTP has been verified
...
//   }
  ...
  void didChangeDependencies() {
    ...
    // Set locale after EasyLocalization is ready (guarantees no race condition)
    final codes = globalStorage.getLanguageCodes();
    if (codes != null && codes.isNotEmpty) {
      final locale = _getLocaleFromLanguageCode(codes.first);
      if (locale != null &&
          context.locale.languageCode != locale.languageCode) {
        debugPrint(
            "OTP Screen (Client) - Setting locale in didChangeDependencies: ${locale.languageCode}");
  ...
  Widget build(BuildContext context) {
    ...
    print("aarati user role varify screen= ${widget.userRole}");
    return Scaffold(
                ...
                BlocConsumer(
                    ...
                    listener: (context, state) {
                      ...
                      if (state is VerifyOTP) {
                        ...
                        globalStorage.saveWolooId(
                            wolooId:
                                verfiyOtpModel!.results!.wolooId.toString());
                        // roleId = globalStorage.getRoleId();
                        debugPrint(
                            "screen role id ----- ${state.verfiyOtpModel!.results!.roleId == 16}");
                        // debugPrint("fcm token id ----- $fcmToken");
                        // updateDeviceToken();
                        // widget.loginBloc.add(
                        //     UpdateTokenOnVerifyOTP(token: fcmToken.toString()));
                        //  state.verfiyOtpModel!.results!.isRegister == 0 ?

                        state.verfiyOtpModel!.results!.roleId == 16
                            ? widget.loginBloc.add(Signup(
                                clientTypeId: 10,
                                userId: state!.verfiyOtpModel!.results!.userId!,
                                mobileNumber: widget.phoneNumber,
                                hostLocation:
                                    state.verfiyOtpModel!.results!.hostLocation,
                                city: state.verfiyOtpModel!.results!.hostCity,
                                hostFacility:
                                    state.verfiyOtpModel!.results!.hostFacility,
                                lat: state.verfiyOtpModel!.results!.lat,
                                long: state.verfiyOtpModel!.results!.lng,
                                address: state.verfiyOtpModel!.results!.address,
                                roleId: state.verfiyOtpModel!.results!.roleId!
                                    .toString(),
                                facilityType: state
                                    .verfiyOtpModel!.results!.facilityType!,
                      ...
                      if (state is LoginUser) {
                        ...
                        // Don't navigate - user is already on verify OTP screen
                        // This state is emitted when resend OTP is clicked
                        EasyLoading.dismiss();
                        EasyLoading.showToast("OTP resent successfully");
                        // Do nothing else - stay on current screen
                        return; // Exit listener early to prevent any other processing
                      }

                      if (state is RegisterUser) {
                        // Only navigate if OTP was actually verified (not just resend)
                        if (!_isOtpVerified) {
                          // This RegisterUser state is from a previous/ongoing signup, ignore it
                          return;
                        }
