...
class SubcriptionBloc extends Bloc<SubscriptionEvent, SubscriptionState> {
  final SubcriptionService subcriptionService =
      SubcriptionService(dio: GetIt.instance());
  var requestId = '';
  late int roleId;
  late int janitorId;
  ...
  FutureOr<void> _mapCreateOrderToState(
    ...
    try {
      ...
      var response = await subcriptionService.creatOrder(
      ...
      emit(CreateOrder(orderModel: response));
    } catch (e) {
  ...
  FutureOr<void> _mapgetUserCoinsToState(
    ...
    try {
      ...
      var response = await subcriptionService.getTask();
      debugPrint("requestId $response");
      response;
      // debugPrint("requestId $requestId");
  ...
  FutureOr<void> _mapgetFacilityStatusToState(
    ...
    try {
      ...
      var response = await subcriptionService.getFacilityStatus(
          clientId: event.clientId, plan: event.plan);
      debugPrint("requestId $response");
      response;
      // debugPrint("requestId $requestId");
  ...
  FutureOr<void> _mapgetPlanToState(
    ...
    try {
      ...
      var response = await subcriptionService.getPlan(
          // event.clientId
          );
